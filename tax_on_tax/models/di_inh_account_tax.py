...
class AccountTax(models.Model):
    ...
    di_tax_id = fields.Many2one(
        'account.tax', string='Tax on tax', help="""Add a tax on the amount of the tax.""")
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True):
                ...
                if tax.price_include or self._context.get('force_price_include'):
                    if tax.amount_type == 'percent':
                        ...
                        tax_amount = tax._compute_amount(
                            base, sign * price_unit, quantity, product, partner) * sum_repartition_factor
                    ...
                    if store_included_tax_total and (
                        tax.amount or tax.amount_type not in (
                            "percent", "division", "fixed")
                    ):
                        total_included_checkpoints[i] = base
                        store_included_tax_total = False
                i -= 1

        total_excluded = currency.round(recompute_base(
            base, incl_fixed_amount, incl_percent_amount, incl_division_amount))
        ...
        for tax in taxes:
            tax_repartition_lines = (is_refund and tax.refund_repartition_line_ids or tax.invoice_repartition_line_ids).filtered(
            ...
            if tax.include_base_amount:
                ...
                subsequent_tags = subsequent_taxes.get_tax_tags(
                    is_refund, 'base')
            ...
            repartition_line_amounts = [round(
                tax_amount * line.factor, precision_rounding=prec) for line in tax_repartition_lines]
            ...
            rounding_error = round(nber_rounding_steps and total_rounding_error /
                                   nber_rounding_steps or 0.0, precision_rounding=prec)
            ...
            # spé ajout taxe sur taxe
            if tax.di_tax_id:
                tax_repartition_lines = (is_refund and tax.di_tax_id.refund_repartition_line_ids or tax.di_tax_id.invoice_repartition_line_ids).filtered(
                    lambda x: x.repartition_type == 'tax')
                sum_repartition_factor = sum(
                    tax_repartition_lines.mapped('factor'))

                price_include = self._context.get(
                    'force_price_include', tax.di_tax_id.price_include)

                # compute the tax_amount
                if price_include and total_included_checkpoints.get(i):
                    # We know the total to reach for that tax, so we make a substraction to avoid any rounding issues
                    di_tax_amount = total_included_checkpoints[i] - (
                        tax_amount + cumulated_tax_included_amount)
                    cumulated_tax_included_amount = 0
                else:
                    di_tax_amount = tax.di_tax_id.with_context(force_price_include=False)._compute_amount(
                        tax_amount, sign * price_unit, quantity, product, partner)

                # Round the tax_amount multiplied by the computed repartition lines factor.
                di_tax_amount = round(di_tax_amount, precision_rounding=prec)
                factorized_tax_amount = round(
                    di_tax_amount * sum_repartition_factor, precision_rounding=prec)
                ...
                subsequent_taxes = self.env['account.tax']
                subsequent_tags = self.env['account.account.tag']
                if tax.di_tax_id.include_base_amount:
                    subsequent_taxes = taxes[i+1:]
                    subsequent_tags = subsequent_taxes.get_tax_tags(
                        is_refund, 'base')
                ...
                # in lines as 2 x 0.01.
                repartition_line_amounts = [round(
                    di_tax_amount * line.factor, precision_rounding=prec) for line in tax_repartition_lines]
                total_rounding_error = round(
                    factorized_tax_amount - sum(repartition_line_amounts), precision_rounding=prec)
                nber_rounding_steps = int(
                    abs(total_rounding_error / currency.rounding))
                ...
                for repartition_line, line_amount in zip(tax_repartition_lines, repartition_line_amounts):
                    ...
                    taxes_vals.append({
                        'id': tax.di_tax_id.id,
                        'name': partner and tax.di_tax_id.with_context(lang=partner.lang).name or tax.di_tax_id.name,
                        'amount': sign * line_amount,
                        'base': round(sign * tax_amount, precision_rounding=prec),
                        'sequence': tax.di_tax_id.sequence,
                        'account_id': tax.di_tax_id.cash_basis_transition_account_id.id if tax.di_tax_id.tax_exigibility == 'on_payment' else repartition_line.account_id.id,
                        'analytic': tax.di_tax_id.analytic,
                        'price_include': price_include,
                        'tax_exigibility': tax.di_tax_id.tax_exigibility,
                        'tax_repartition_line_id': repartition_line.id,
                        'group': groups_map.get(tax),
                        'tag_ids': (repartition_line.tag_ids + subsequent_tags).ids,
                        'tax_ids': subsequent_taxes.ids,
                ...
                # Affect subsequent taxes
                if tax.di_tax_id.include_base_amount:
                    tax_amount += factorized_tax_amount
